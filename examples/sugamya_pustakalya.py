...
##        print("\t**********************************************")
    ...
    def get_latest_books(self):
        ...
        window=self.pcs._event.app.editor.find_window(self.WINDOW)
#        _windows=self.pcs._event.app.editor._windows
                 
        window.buffer = "buffer text entered"
        window.emit(EventType.TEXT_CHANGED)
        
         


#        c_win=self.pcs._event.app.editor.current_window #"str(self.pcs._event.app.editor.current_window)"
#        self.pcs._event.app.editor.message(str(c_win)+"<--c_win,buffer_str:-->"+str(c_win.buffer))        
        
        
        page=self.get_latest_page=+1
        ...
        if(data.status_code == 200):       
            ...
            if(len(books) == 0):
                self.pcs._event.app.editor.message("No book found")
            else:
